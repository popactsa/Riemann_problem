...
inline int
count_time_between(std::chrono::time_point<std::chrono::system_clock> &from,
...
template <typename timeformat>
    requires std::common_with<timeformat, std::chrono::seconds>
inline int count_time_between_const(
...
get_path_to_file_in_dir(const std::filesystem::path &, int, std::string_view);

inline std::tuple<solver_types, std::filesystem::path>
...
template <typename C>
    requires custom::concepts::Container<C> &&
             (!std::assignable_from<typename C::value_type, std::string_view>)
constexpr void split_string(
    std::string_view init, C &result,
    const char sep) // if get rid of expect-assertions, function can be declared
                    // constexpr
{
    ...
    for (std::size_t current(init.find(sep, prev)); current != init.npos;
        ...
        expect<Error_action::throwing, std::length_error>(
            [&]() {
                return (std::distance(
                            it, const_cast<C::iterator>(result.end())) > 0);
            },
            (std::string("Size of init string is too small, must be = ") +
             std::to_string(std::size(result)))
                .c_str());
    ...
    *it++ = init.substr(prev);
    expect<Error_action::throwing, std::length_error>(
        [&]() {
            return (std::distance(it, const_cast<C::iterator>(result.end())) ==
                    0);
        },
        (std::string("Size of init string is too large, must be = ") +
         std::to_string(std::size(result)))
            .c_str());
}
...
template <typename C>
    requires custom::concepts::Container<C> &&
             (!std::assignable_from<typename C::value_type, std::string_view>)
constexpr void split_string(std::string_view init,
                            C &result) // if get rid of expect-assertions,
                                       // function can be declared constexpr
{
...
inline void fmt_add_align(std::string &fmt, const std::string &align,
                          const std::vector<int> &sizes) noexcept
{
    for (auto it : sizes) {
        fmt += "{:";
        fmt += align;
...
inline void fmt_add_align(
    std::string &fmt,
    const std::vector<std::pair<std::string, int>> &align_n_sizes) noexcept
{
    for (auto [align, size] : align_n_sizes)
        fmt_add_align(fmt, align, {size});
}

std::string get_format_by_left_side_impl(
    std::initializer_list<std::string_view> args) noexcept; // deprecate?
#endif
