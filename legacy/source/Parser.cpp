...
void Parser::parse_line(
    ...
    parsing_tbl_type* parsing_tbl,
    initialized_variables_type* initialized_variables,
    get_object_variable_info_function_type* get_object_variable_info,
    assign_enum_variable_function_type* assign_enum_variable_function)
{
    ...
    if (read[0]
        == '#' // commentary
        || read[0]
        == '!') // info is not meant to be parsed into a variable
    {
        return;
    }
    ...
    variable_info_type variable = get_read_variable_info(
        read_split, object, parsing_tbl, get_object_variable_info);
    void* variable_ptr;
    std::string_view variable_type;
    Parameters_common::OBJECT_GROUP variable_supposed_object_group;
    std::tie(variable_type, variable_supposed_object_group, variable_ptr) =
        variable;

    if (variable_supposed_object_group
        != NONE
        && std::get<Parameters_common::OBJECT_GROUP>(object)
        == NONE) // i.e. its a group section beginning
    {
        object = {variable_supposed_object_group,
                  find_object_ptr(variable_ptr, read_split)};
        return;
    }
    assign_read_variable(variable, read_split[1], get_object_variable_info,
                         assign_enum_variable_function);
    initialized_variables->emplace(variable_ptr);
}

void* Parser::find_object_ptr(void* object_ptr_array_ptr,
                              const std::vector<std::string_view>& read_split)
{
    int offset;
    std::from_chars(read_split[1].data(),
                    read_split[1].data() + read_split[1].size(), offset);
    return static_cast<char*>(object_ptr_array_ptr) + offset;
}

void Parser::assign_read_variable(
    variable_info_type& variable,
    ...
    assign_enum_variable_function_type* assign_enum_variable_function)
{
    ...
    if (!variable_type.compare("double")) {
        double value;
        std::from_chars(read_value.data(),
                        read_value.data() + read_value.size(), value);
...
Parser::get_read_variable_info(
    const std::vector<std::string_view>& read_split,
    const object_info_type& object,
    parsing_tbl_type* parsing_tbl,
    get_object_variable_info_function_type* get_object_variable_info)
{
