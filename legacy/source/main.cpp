...
enum class solver_type { solver_Lagrange_1D, solver_elasticity_Lagrange_1D };
...
int main()
{
    ...
    if (check_rmod(scenario_dir)) {
        std::cout << "Choose scenario from " << scenario_dir << " : "
                  << std::endl;
        ...
        auto [scenario_solver_type, scenario_file] =
            get_path_to_file_in_dir(scenario_dir, choose_item, postfix);
        ...
        std::visit(overloaded{[](std::monostate &sol) {
